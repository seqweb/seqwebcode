...
class RunCommand(BaseCommand):
  ...
  Examples:
    ...
    def run(self, args: List[str]):
        """Override run method to handle unknown arguments properly"""
        # First argument is the command name, skip it for argparse
        parser = self.create_parser()
        parsed_args, unknown_args = parser.parse_known_args(args[1:])
        # Store unknown args for the target program
        parsed_args.args = unknown_args
    ...
    def _run_python_file(self, file_path: Path, program_args: list, noisy: bool = False):
        ...
        try:
            ...
            python_src_dir = cli_dir.parent.parent  # Go up from cli/commands/ to src/python/
            ...
            if noisy:
                ...
                if result.returncode == 0:
                    ...
                    print(f"⚠️  Program completed with exit code: {result.returncode}")
